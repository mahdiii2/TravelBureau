...
@app.get('/get-hotel-contracts', response_model=list[schemas.HotelContract])
def get_hotel_contracts(db: Session = Depends(get_db)):
    return crud.get_hotel_contracts(db)


@app.post('/upload-file', response_model=schemas.HotelContractJob)
def upload_file(country: str = Form(...), hotel: str = Form(...), file: UploadFile = File(...), db: Session = Depends(get_db)):
    if not file.filename.endswith(('.xlsx', '.xls')):
        raise HTTPException(status_code=400, detail='Invalid file type')
    file_path = Path('uploads') / f"{uuid4()}-{file.filename}"
...
@app.get('/get-hotel-contract-jobs', response_model=list[schemas.HotelContractJob])
